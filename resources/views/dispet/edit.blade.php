<x-app-layout>
    ...
    <div class="container mt-5">
        <form action="{{ route('dispet.update', $dispet->id_avto) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <select class="form-control" id="id_avto" name="id_avto" required>
                    @foreach($avtos as $avto)
                        <option value="{{ $avto->id_avto }}" {{ $avto->id_avto == $dispet->id_avto ? 'selected' : '' }}>
                            {{ $avto->id_avto }} - {{ $avto->name_avto }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <input type="datetime-local" class="form-control" id="time_prib" name="time_prib" value="{{ $dispet->time_prib }}" required>
            </div>
            <div class="mb-3">
                <label for="time_otb" class="form-label">Время отбытия</label>
                <input type="datetime-local" class="form-control" id="time_otb" name="time_otb" value="{{ $dispet->time_otb }}" required>
            </div>
            <div class="mb-3">
                ...
                <select class="form-control" id="id_vod" name="id_vod" required>
                    @foreach($voditeli as $voditel)
                        <option value="{{ $voditel->id_vod }}" {{ $voditel->id_vod == $dispet->id_vod ? 'selected' : '' }}>
                            {{ $voditel->id_vod }} - {{ $voditel->name_vod }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-control" id="pytevka" name="pytevka" required>
                    @foreach($marhs as $marh)
                        <option value="{{ $marh->id_marh }}" {{ $marh->id_marh == $dispet->pytevka ? 'selected' : '' }}>
                            {{ $marh->id_marh }} - {{ $marh->mesto }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-control" id="tovar" name="tovar" required>
                    @foreach($tovars as $tovar)
                        <option value="{{ $tovar->id_tovar }}" {{ $tovar->id_tovar == $dispet->tovar ? 'selected' : '' }}>
                            {{ $tovar->id_tovar }} - {{ $tovar->name_tovar }}
                        </option>
                    @endforeach
                </select>
            </div>
