<x-app-layout>
    ...
    <div class="container mt-5">
        <form action="{{ route('garage.update', $garage->id_avto) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <select class="form-control" id="id_avto" name="id_avto" required>
                    @foreach($avtos as $avto)
                        <option value="{{ $avto->id_avto }}" {{ $avto->id_avto == $garage->id_avto ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="polomka" name="polomka" value="{{ $garage->polomka }}" required>
            </div>
            <div class="mb-3">
                <label for="zapchast" class="form-label">Запчасть</label>
                <input type="text" class="form-control" id="zapchast" name="zapchast" value="{{ $garage->zapchast }}" required>
            </div>
            <div class="mb-3">
                <label for="cena_zapchast" class="form-label">Цена Запчасти</label>
                <input type="text" class="form-control" id="cena_zapchast" name="cena_zapchast" value="{{ $garage->cena_zapchast }}" required>
            </div>
            <div class="mb-3">
                <label for="data_nachalo" class="form-label">Дата Начала</label>
                <input type="date" class="form-control" id="data_nachalo" name="data_nachalo" value="{{ $garage->data_nachalo }}" required>
            </div>
            <div class="mb-3">
                <label for="data_konec" class="form-label">Дата Конца</label>
                <input type="date" class="form-control" id="data_konec" name="data_konec" value="{{ $garage->data_konec }}" required>
            </div>
